...
function AlienLaunchMarker:init(world)
    ...
    -- checks if player collided already with something
...
function AlienLaunchMarker:update(dt)
    ...
    if self.launched and not self.collisionDetected then
        if love.keyboard.wasPressed('space') then
            for i = 1, 2 do

                -- create random vector impulse
                local angle = math.random(80, 120) / 100
                local vecImpulseX, vecImpulseY = self.alien.body:getLinearVelocity()
                vecImpulseX = vecImpulseX * angle
                vecImpulseY = vecImpulseY * angle

                -- get current position of the main alien
                ...
                -- create a new alien with the same position as the main one
                local alien = Alien(self.world, 'round', x, y, 'Player')

                -- apply the difference between current X,Y and base X,Y as launch vector impulse
                alien.body:setLinearVelocity(vecImpulseX, vecImpulseY)
